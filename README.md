...
I build **synthesizable, high-performance RISC-V cores** from scratch — focusing on advanced pipelining, superscalar execution, branch prediction, MMU, and vector extensions.

- ⚡ Currently working on **Linux-bootable multi-core RISC-V SoCs**
- 🔬 Exploring **RVV vector units**, AI accelerators & low-power optimization
- 🛠️ Tools: SystemVerilog, Verilog, Vivado, Verilator, ModelSim, GTKWave
...
**HDL** `SystemVerilog` `Verilog` `RISC-V Assembly`  

**EDA & Simulation** `Vivado` `ModelSim` `Verilator` `GTKWave` `Icarus`  

**Architecture** `RISC-V` `Superscalar` `Branch Prediction` `MMU` `RVV Vector` `SoC Design` `Low-Power`  

**Others** `Python` `C` `CUDA`
